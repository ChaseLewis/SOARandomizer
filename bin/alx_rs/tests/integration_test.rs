...
macro_rules! skip_if_no_reference {
    () => {
        if !Path::new(REFERENCE_DIR).exists() {
            eprintln!("Skipping: Reference directory not found at {}", REFERENCE_DIR);
            return;
        }
    };
}
...
fn test_binary_runs_successfully() {
    ...
    assert!(stdout.contains("Export complete!"), "Missing completion message");
...
fn test_binary_default_output_dir() {
    ...
    assert!(stdout.contains("Output directory"), "Help should mention output directory");
...
fn test_binary_version_flag() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("alx_rs") || stdout.contains("0.1"), 
