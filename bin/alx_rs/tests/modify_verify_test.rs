...
fn test_modify_character_stats() {
    let Some(test_iso) = create_test_iso("modify_character") else { return };
    ...
    game.write_characters(&modified_chars).expect("Failed to write characters");
    ...
    let reread_chars = game2.read_characters().expect("Failed to re-read characters");
    ...
    assert_eq!(reread_chars[0].name, original_chars[0].name, "Vyse name changed unexpectedly");
    assert_eq!(reread_chars[0].age, original_chars[0].age, "Vyse age changed unexpectedly");
...
fn test_modify_accessory_stats() {
    let Some(test_iso) = create_test_iso("modify_accessory") else { return };
    
    let mut game = GameRoot::open(&test_iso).expect("Failed to open ISO");
    let original = game.read_accessories().expect("Failed to read accessories");
    
    println!("Original first accessory: {} - Buy: {}", original[0].name, original[0].buy_price);
    ...
    println!("After modification: {} - Buy: {}", reread[0].name, reread[0].buy_price);
...
fn test_modify_weapon_stats() {
    let Some(test_iso) = create_test_iso("modify_weapon") else { return };
    
    let mut game = GameRoot::open(&test_iso).expect("Failed to open ISO");
    let original = game.read_weapons().expect("Failed to read weapons");
    
    println!("Original first weapon: {} - Attack: {}", original[0].name, original[0].attack);
    ...
    println!("After modification: {} - Attack: {}", reread[0].name, reread[0].attack);
...
fn test_modify_shop_items() {
    let Some(test_iso) = create_test_iso("modify_shop") else { return };
    
    let mut game = GameRoot::open(&test_iso).expect("Failed to open ISO");
    let original = game.read_shops().expect("Failed to read shops");
    
    println!("Original first shop items: {:?}", &original[0].item_ids[..5]);
...
fn test_modify_treasure_chest() {
    let Some(test_iso) = create_test_iso("modify_chest") else { return };
    
    let mut game = GameRoot::open(&test_iso).expect("Failed to open ISO");
    let original = game.read_treasure_chests().expect("Failed to read chests");
    
    println!("Original first chest: item_id={}, amount={}", 
    ...
    game.write_treasure_chests(&modified).expect("Failed to write");
    game.save_dol().expect("Failed to save");
    drop(game);
    ...
    println!("After modification: item_id={}, amount={}", 
...
fn test_csv_import_workflow() {
    ...
    let Some(test_iso) = create_test_iso("csv_import_workflow") else { return };
    
    // Step 1: Read original values
    let mut game = GameRoot::open(&test_iso).expect("Failed to open ISO");
    let original_chars = game.read_characters().expect("Failed to read characters");
    
    println!("Original Vyse: Max HP={}, Power={}", original_chars[0].max_hp, original_chars[0].power);
    
    // Step 2: Export to CSV
    let mut csv_data = Vec::new();
    CsvExporter::export_characters(&original_chars, &mut csv_data).expect("Failed to export");
    ...
    println!("  Vyse Max HP: {}, Power: {}", chars_to_modify[0].max_hp, chars_to_modify[0].power);
    println!("  Aika Max HP: {}, Power: {}", chars_to_modify[1].max_hp, chars_to_modify[1].power);
    
    // Step 4: Write and save
    game.write_characters(&chars_to_modify).expect("Failed to write");
    game.save_dol().expect("Failed to save");
    drop(game);
    ...
    assert_eq!(final_chars[0].max_hp, 7777, "Vyse max_hp not persisted from CSV");
    assert_eq!(final_chars[0].power, 111, "Vyse power not persisted from CSV");
    assert_eq!(final_chars[1].max_hp, 6666, "Aika max_hp not persisted from CSV");
    assert_eq!(final_chars[1].power, 222, "Aika power not persisted from CSV");
