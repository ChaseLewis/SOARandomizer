...
/// Category ID for enemy skills.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[repr(i8)]
pub enum EnemySkillCategory {
    SMove = 0,
    Magic = 1,
}

impl EnemySkillCategory {
...
/// An enemy super move (S-Move) or magic spell.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EnemySuperMove {
    /// Entry ID
    pub id: u32,
    /// Move name
    pub name: String,
    /// Category ID (0=S-Move, 1=Magic)
    pub category_id: i8,
    /// Effect ID
    pub effect_id: i8,
    /// Scope ID
    pub scope_id: u8,
    /// Effect parameter ID (can be trait, HP, or enemy ID depending on effect)
...
impl EnemySuperMove {
    ...
    /// 17 + 4(pad) + 1 + 1 + 1 + 2 + 2 + 1 + 1 + 1 + 1 + 1 + 1 + 2(pad) = 36 bytes
    pub const ENTRY_SIZE_US_JP: usize = 36;

    /// Size of one entry in bytes (EU - extra padding byte after scope).
    pub const ENTRY_SIZE_EU: usize = 34;

    /// Read a single enemy super move from binary data.
    ...
    /// Read all enemy super move entries from binary data.
    pub fn read_all_data(data: &[u8], version: &GameVersion) -> Result<Vec<Self>> {
        let mut entries = Vec::new();
        let mut cursor = Cursor::new(data);

        let id_range = id_ranges::ENEMY_SUPER_MOVE_GC;
    ...
    pub fn category_name(&self) -> &'static str {
        EnemySkillCategory::from_id(self.category_id).name()
    }
    ...
    /// Write a single enemy super move to binary data.
...
mod tests {
    ...
    #[test]
    fn test_entry_size() {
        assert_eq!(EnemySuperMove::ENTRY_SIZE_US_JP, 36);
    }
}
