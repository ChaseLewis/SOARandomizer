...
pub fn parse_enp(data: &[u8], filename: &str, version: &GameVersion) -> Result<EnpData> {
    ...
    if sig == FILE_SIG {
        ...
        if check != -1 {
            return Err(Error::ParseError { offset: 4, message: "ENP segments corrupted".to_string() });
        }
...
fn parse_enp_segment(data: &[u8], filename: &str, version: &GameVersion, result: &mut EnpData) -> Result<()> {
    if data.len() < 8 {
        return Ok(());
    }
    ...
    for node in &nodes {
        ...
        match Enemy::read_one(&mut cursor, node.id, filename, version) {
            Ok(enemy) => {
                ...
                // Read tasks
                let mut task_id = 1u32;
                let remaining = data.len().saturating_sub(node.pos + cursor.position() as usize);
                let max_tasks_possible = remaining / EnemyTask::ENTRY_SIZE;
