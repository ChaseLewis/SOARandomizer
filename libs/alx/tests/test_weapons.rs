...
fn test_weapon_cutlass() {
    ...
    println!(
        "  Attack: {}, Hit%: {}",
        cutlass.attack, cutlass.hit_percent
    );
}
...
fn test_weapon_effects() {
    ...
    let weapons_with_effects: Vec<_> = weapons.iter().filter(|w| w.effect_id >= 0).collect();
    ...
    assert!(
        !weapons_with_effects.is_empty(),
        "Some weapons should have effects"
    );
}
