...
use alx::entries::{
    ...
    ShipAccessory, ShipItem, Swashbuckler,
};
...
macro_rules! patch_roundtrip_test {
    ($test_name:ident, $entry_type:ty, $data_field:ident) => {
        ...
        fn $test_name() {
            ...
            assert_eq!(crc_orig, crc_out, concat!(stringify!($entry_type), " patch roundtrip CRC32 mismatch"));
        }
    };
...
patch_roundtrip_test!(test_character_magic_roundtrip, CharacterMagic, character_magic_data);
patch_roundtrip_test!(test_character_super_move_roundtrip, CharacterSuperMove, character_super_move_data);
patch_roundtrip_test!(test_crew_member_roundtrip, CrewMember, crew_member_data);
patch_roundtrip_test!(test_ship_accessory_roundtrip, ShipAccessory, ship_accessory_data);
...
patch_roundtrip_test!(test_character_roundtrip, Character, character_data, no_version);
patch_roundtrip_test!(test_playable_ship_roundtrip, PlayableShip, playable_ship_data, no_version);
patch_roundtrip_test!(test_shop_roundtrip, Shop, shop_data, no_version);
patch_roundtrip_test!(test_treasure_chest_roundtrip, TreasureChest, treasure_chest_data, no_version);
