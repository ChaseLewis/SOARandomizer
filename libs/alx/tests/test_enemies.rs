...
fn test_read_enemies() {
    ...
    println!(
        "✓ Read {} enemies ({} global, {} file-specific) and {} tasks",
        enemies.len(),
        global_count,
        file_specific,
        tasks.len()
...
fn test_enemy_filter_breakdown() {
    ...
    for enemy in enemies.iter().take(10) {
        println!(
            "  ID {}: filter='{}', name='{}'",
            enemy.id, enemy.filter, enemy.name_jp
        );
    }

    // Check for ecinit specifically
    let ecinit_count = enemies
        .iter()
        .filter(|e| e.filter.contains("ecinit"))
        .count();
    let ebinit_count = enemies
        .iter()
        .filter(|e| e.filter.contains("ebinit"))
        .count();
    println!(
        "\nDAT breakdown: ecinit={}, ebinit={}",
        ecinit_count, ebinit_count
    );

    // List all unique ecinit/ebinit files
    let mut dat_files: Vec<_> = enemies
        .iter()
...
fn test_debug_id_165() {
    ...
    use alx::io::{decompress_aklz, parse_dat_file, parse_evp};
...
fn test_enemy_tasks() {
    ...
    for task in &tasks {
        assert!(
            task.type_id >= -1 && task.type_id <= 1,
            "Task type ID should be -1, 0, or 1, got {}",
            task.type_id
        );
    }
