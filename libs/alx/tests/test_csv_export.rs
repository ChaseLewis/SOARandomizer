...
macro_rules! skip_if_no_reference {
    () => {
        if !Path::new(REFERENCE_CSV_DIR).exists() {
            eprintln!(
                "Skipping: Reference CSV directory not found at {}",
                REFERENCE_CSV_DIR
            );
            return;
        }
...
fn test_weapon_csv_matches_reference() {
    ...
    for (weapon, r) in weapons.iter().zip(refs.iter()) {
        ...
        assert_eq!(
            weapon.trait_data.value,
            r.i16("Trait Value"),
            "{ctx}: Trait Value"
        );
        assert_eq!(
            weapon.description_pos,
            r.hex("[US Descr Pos]"),
            "{ctx}: Descr Pos"
        );
        assert_eq!(
            weapon.description_size,
            r.u32("[US Descr Size]"),
            "{ctx}: Descr Size"
        );
    }
...
fn test_accessory_csv_matches_reference() {
    ...
    for (acc, r) in accessories.iter().zip(refs.iter()) {
        ...
        assert_eq!(
            acc.character_flags.0 as u16,
            r.binary_flags("PC Flags"),
            "{ctx}: PC Flags"
        );
    ...
    println!(
        "✓ All {} accessories match reference CSV!",
        accessories.len()
    );
}
...
fn test_armor_csv_matches_reference() {
    ...
    for (armor, r) in armors.iter().zip(refs.iter()) {
        ...
        assert_eq!(
            armor.character_flags.0 as u16,
            r.binary_flags("PC Flags"),
            "{ctx}: PC Flags"
        );
        ...
        assert_eq!(
            armor.traits[0].value,
            r.i16("Trait 1 Value"),
            "{ctx}: Trait 1 Value"
        );
        assert_eq!(
            armor.description_pos,
            r.hex("[US Descr Pos]"),
            "{ctx}: Descr Pos"
        );
    }
...
fn test_character_magic_csv_matches_reference() {
    ...
    println!(
        "✓ All {} character magics match reference CSV!",
        magics.len()
    );
}
...
fn test_treasure_chest_csv_matches_reference() {
    ...
    println!(
        "✓ All {} treasure chests match reference CSV!",
        chests.len()
    );
}
...
fn test_ship_accessory_csv_matches_reference() {
    ...
    println!(
        "✓ All {} ship accessories match reference CSV!",
        accessories.len()
    );
}
...
fn test_enemy_super_move_csv_matches_reference() {
    ...
    println!(
        "✓ All {} enemy super moves match reference CSV!",
        moves.len()
    );
}
...
fn test_character_super_move_csv_matches_reference() {
    ...
    assert_eq!(
        moves.len(),
        refs.len(),
        "Character super move count mismatch"
    );
    ...
    println!(
        "✓ All {} character super moves match reference CSV!",
        moves.len()
    );
}
...
fn compare_csv_output(generated: &str, reference_path: &str) -> Result<(), String> {
    ...
    let gen_records: Vec<csv::StringRecord> = gen_reader.records().filter_map(|r| r.ok()).collect();
    let ref_records: Vec<csv::StringRecord> = ref_reader.records().filter_map(|r| r.ok()).collect();
...
fn compare_csv_deep_skip(
    ...
    // Get headers
    let gen_headers: Vec<String> = gen_reader
        .headers()
    ...
    let common_cols: Vec<(usize, usize, &String)> = gen_headers
        .iter()
        .enumerate()
...
fn compare_csv_deep(generated: &str, reference_path: &str, id_column: &str) -> Result<(), String> {
...
fn test_csv_exporter_accessory_row_count() {
    ...
    compare_csv_output(&generated, "accessory.csv").expect("Accessory CSV comparison failed");
...
fn test_csv_exporter_armor_row_count() {
    ...
    compare_csv_output(&generated, "armor.csv").expect("Armor CSV comparison failed");
...
fn test_csv_exporter_weapon_row_count() {
    ...
    compare_csv_output(&generated, "weapon.csv").expect("Weapon CSV comparison failed");
...
fn test_csv_exporter_usable_item_row_count() {
    ...
    compare_csv_output(&generated, "usableitem.csv").expect("Usable item CSV comparison failed");
...
fn test_csv_exporter_special_item_row_count() {
    ...
    compare_csv_output(&generated, "specialitem.csv").expect("Special item CSV comparison failed");
...
fn test_csv_exporter_shop_row_count() {
    ...
    compare_csv_output(&generated, "shop.csv").expect("Shop CSV comparison failed");
